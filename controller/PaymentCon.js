    ...
    return{
        ...
        uplive: async (req,res)=>{
            try {
                ...
                if(data.ID !== "no store ID" && data.pass !== "no store password" && data.amount !== 0){
                ...
                res.redirect("/paycon");
                    ...
                    const data3 = await Payment.updateOne({name:"payment"},{live:false});
                    res.render("payset",{
                        data,
        ...
        upStatus: async (req,res)=>{
            try {
                ...
                if(data.ID !== "no store ID" && data.pass !== "no store password" && data.amount !== 0){
                    ...
                    const data2 = await Payment.updateOne({name:"payment"},{status});
                    
                res.redirect("/paycon");
                }
                else{
                    const data3 = await Payment.updateOne({name:"payment"},{status:false});
                    res.render("payset",{
                        data,
                        err:"Please Add Gateway credentials"
                    })
